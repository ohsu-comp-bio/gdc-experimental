        ...
        <div className="row">
          <div className="col-sm-3 col-md-2 sidebar">
            <ul className="nav nav-sidebar">
              <li className="active"><a href="#">Filters... <span className="sr-only">(current)</span></a></li>
            ...
            <nav>
              <div className="input-group">
                ...
                <span className="input-group-btn">
                  <button className="btn btn-default" type="button"><span className="fa fa-search" aria-hidden="true"></span></button>
                </span>
              </div>
            ...
            <table className="table">
              <thead>
                ...
                {users.length > 0 &&
                  <tbody>
                      {users.map((user,i) =>
                        <tr key={user.html_url}>
                          <td scope="row">{i}</td>
                          <td>
                            <a href={user.html_url} target="_blank">
                              <img src={user.avatar_url} width="32" height="32" alt="owner" />
                              <span style={{marginLeft: '0.5em'}}>{user.login}</span>
                            </a>
                          </td>
                          <td>
                            <a href={user.followers_url} target="_blank">
                              <span style={{marginLeft: '0.5em'}}>followers</span>
                            </a>, &nbsp;
                            <a href={user.organizations_url} target="_blank">
                              <span style={{marginLeft: '0.5em'}}>orgs</span>
                            </a>
                          </td>
                        </tr>
                      )}
