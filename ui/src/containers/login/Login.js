...
class Login extends Component {
  ...
  handleLogin(event) {
    ...
    this.props.dispatch(login(username.value, password.value, token.value));
    username.value = '';
    password.value = '';
  }
            ...
            <div className="card">
              ...
              <form className="card-block">
                ...
                <div className="input-group">
                  <span className="input-group-addon"><i className="fa fa-code"/></span>
                  <textarea  ref="token" className="form-control" placeholder="JWT Token" required autoFocus>
                  {
                    this.props.jwt_token
                  }
                  </textarea>
                </div>

                <div className="row-fluid">
                    <div className="span12 text-xl-center">
                    -OR-
                    </div>
                </div>
...
function mapStateToProps(state) {
  const { auth } = state;
  if (auth) {
    return { user: auth.user, loginError: auth.loginError, jwt_token: auth.jwt_token };
  }
