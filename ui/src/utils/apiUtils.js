...
export function callApi(url, config, request, onRequestSuccess, onRequestFailure) {
  ...
  config.headers.authorization = "Bearer "+ loadJWTToken() ;
  return dispatch => {
    dispatch(request);
...
export function loadUserProfile() {
  try {
   ...
   userProfile.jwt_token = loadJWTToken();
    ...
    // return the jwt token if it was stored
    return {jwt_token: loadJWTToken()};
  }
}

export const JWT_TOKEN = 'jwt_token';

export function setJWTToken(idToken) {
